  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f0f0f', display: 'flex', flexDirection: 'column', padding: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: '1.5rem' }}>
        <button
          onClick={onCreate}
          style={{ flex: 1, background: '#E8192C', color: '#fff', border: 'none', borderRadius: 14, padding: '16px 10px', fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', letterSpacing: '0.3px', boxShadow: '0 4px 20px rgba(232,25,44,0.35)' }}
        >
          ➕ Créer un défi
        </button>
        <button
          onClick={() => { setShowJoin(v => !v); setJoinError('') }}
          style={{ flex: 1, background: 'transparent', color: showJoin ? '#E8192C' : '#fff', border: `1.5px solid ${showJoin ? '#E8192C' : '#333'}`, borderRadius: 14, padding: '16px 10px', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
        >
          🔗 Rejoindre
        </button>
      </div>
        ...
        <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 14, padding: '1rem', marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              style={{ background: '#E8192C', color: '#fff', border: 'none', borderRadius: 10, padding: '11px 16px', fontSize: 14, fontWeight: 700, cursor: 'pointer', flexShrink: 0, opacity: joinLoading ? 0.6 : 1, fontFamily: 'inherit' }}
            >
              {joinLoading ? '...' : 'OK'}
            </button>
          </div>
          {joinError && <p style={{ color: '#E8192C', fontSize: 12, marginTop: 8 }}>{joinError}</p>}
        </div>
      )}
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', paddingTop: '1.5rem' }}>
            ...
            <p style={{ color: '#555', fontSize: 14, lineHeight: 1.6 }}>
              Invite tes amis.<br />
              <span style={{ color: '#E8192C', fontWeight: 600 }}>Qui lâche en premier ?</span>
            </p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {defis.map(defi => {
              ...
              return (
                ...
                >
                  <div style={{ width: 46, height: 46, borderRadius: 12, background: ended ? '#222' : 'rgba(232,25,44,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0, border: ended ? '1px solid #2a2a2a' : '1px solid rgba(232,25,44,0.2)' }}>
                    {challenge.icon}
                  </div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ color: '#555', fontSize: 11, marginTop: 2 }}>
                      {challenge.label}
                      {left !== null && (
                        <span style={{ marginLeft: 8, color: ended ? '#444' : left <= 3 ? '#E8192C' : '#666' }}>
                  ...
                  <button
                    ...
                    style={{ background: ended ? '#1f1f1f' : '#E8192C', color: ended ? '#555' : '#fff', border: ended ? '1px solid #2a2a2a' : 'none', borderRadius: 10, padding: '9px 14px', fontSize: 13, fontWeight: 700, cursor: ended ? 'default' : 'pointer', flexShrink: 0, fontFamily: 'inherit' }}
                  >
                    {ended ? 'Voir' : 'Entrer →'}
                  </button>
                </div>
